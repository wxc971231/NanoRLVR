...
# -*- coding: utf-8 -*-
"""
One-file GRPO (outcome reward) training + evaluation on GSM8K.
- Minimal "NanoGPT-like" loop: rollout -> reward -> GRPO loss -> update -> eval
- Works with torchrun DDP
- Optional: KL to reference model, 8bit optimizer, LoRA

Example:
torchrun --nproc_per_node=6 grpo_gsm8k_onefile.py \
  --model_path /path/to/Qwen_or_Llama \
  --train_jsonl data/gsm8k/train.jsonl \
  --test_jsonl  data/gsm8k/test.jsonl \
  --total_steps 2000 --batch_size 8 --group_size 8 --max_new_tokens 256 \
  --kl_beta 0.02 --clip_eps 0.2 --k_epochs 2 --lr 2e-6
